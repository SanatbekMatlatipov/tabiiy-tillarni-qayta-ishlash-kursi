...
class FineTunedClassifier:
    ...
    def _fit_bert(self, texts, labels, model_name, epochs, batch_size, lr) -> None:
        ...
        # Kichik korpusda warmup umumiy qadamlar sonidan oshib ketmasligi kerak
        total_steps = max(1, (len(texts) // max(1, batch_size)) * epochs)
        ...
        # transformers versiyalari o'rtasida moslik:
        #   >= 5.x  -> Trainer(processing_class=...)
        #   4.x     -> Trainer(tokenizer=...)
        #   ikkalasi ham rad etsa -> data_collator yetarli, tokenizersiz quriladi
        base_kwargs = dict(
            model=self._model,
            args=args,
            train_dataset=ds,
        ...
        try:
            trainer = Trainer(processing_class=self._tokenizer, **base_kwargs)
        except TypeError:
            try:
                trainer = Trainer(tokenizer=self._tokenizer, **base_kwargs)
            except TypeError:
